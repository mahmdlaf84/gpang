...
from sky.adaptors import aliyun
...
@registry.CLOUD_REGISTRY.register(aliases=['alibaba-cloud', 'alibaba'])
class Aliyun(clouds.Cloud):
    """Alibaba Cloud."""

    _REPR = 'Aliyun'
    _MAX_CLUSTER_NAME_LEN_LIMIT = 96
    _CLOUD_UNSUPPORTED_FEATURES = {
        clouds.CloudImplementationFeatures.MULTI_NODE:
            'Multi-node clusters are not yet supported on Alibaba Cloud.',
        clouds.CloudImplementationFeatures.SPOT_INSTANCE:
            'Spot instances require bespoke bidding logic.',
        clouds.CloudImplementationFeatures.CUSTOM_DISK_TIER:
            'Custom disk tiers are not exposed in the catalog.',
        clouds.CloudImplementationFeatures.STOP:
            'Stopping instances is not supported by the current implementation.',
        clouds.CloudImplementationFeatures.OPEN_PORTS:
            'Firewall management is not automated for Alibaba Cloud.',
    }
    ...
    def get_egress_cost(self, num_gigabytes: float) -> float:
        # Alibaba Cloud offers 0.12 USD/GB public egress as an approximate default.
    ...
    def make_deploy_resources_variables(
        ...
        if instance_type is None:
            raise exceptions.NotSupportedError(
                'Alibaba Cloud requires an explicit instance_type.')
    ...
    def _check_credentials(cls) -> Tuple[bool, Optional[str]]:
        ...
        if missing:
            return (False,
                    'Alibaba Cloud credentials not configured. Set environment '
        ...
        try:
            client = aliyun.create_client(region, access_key, secret)
            from aliyunsdkecs.request.v20140526.DescribeRegionsRequest import \
                DescribeRegionsRequest
            request = DescribeRegionsRequest()
            request.set_accept_format('json')
            client.do_action_with_exception(request)
        except aliyun.exceptions() as exc:  # type: ignore[arg-type]
